...
while True:
    ...
    if table_contour is not None:
        ...
        # Detect the pink paper within the white table area
        pink_paper_rect, pink_paper_box = detect_pink_paper(frame, table_mask)

        if pink_paper_box is not None:
            # Draw a green rotated rectangle around the pink paper
            cv2.drawContours(frame, [pink_paper_box], 0, (0, 255, 0), 2)
            # Draw a red dot at the center of the pink paper
            center_of_pink_paper = (int(pink_paper_rect[0][0]), int(pink_paper_rect[0][1]))
            cv2.circle(frame, center_of_pink_paper, 5, (0, 0, 255), -1)
