...
def detect_balls(frame, table_contour, origin, y_direction):
    ...
    detect_and_draw_balls(frame, black_mask, table_mask, (255, 255, 0), origin, y_direction)
    ...
    for contour in contours:
        ...
        if cv2.contourArea(contour) > 100:  # Threshold area
            ...
            if origin is not None:
                ...
                angle_sign = "-" if cross_product_z < 0 else "+"

                cv2.putText(frame, f"{angle_sign}{angle_degrees:.0f} degree", (center[0] - 40, center[1] - 40), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (100, 100, 255), 2)
...
while True:
    ...
    frame = cv2.GaussianBlur(frameOrigin, (7, 7), 0)
        ...
        if pink_paper_box is not None:
            ...
            for spot in yellow_spots:
                ...
                if yellow_center:
                    ...
                    y_direction = (yellow_center[0] - pink_paper_center[0], yellow_center[1] - pink_paper_center[1])
                    break  # Assuming the first detected yellow spot as the Y-axis point

        if pink_paper_center is not None and y_direction is not None:
            detect_balls(frame, table_contour, pink_paper_center, y_direction)
